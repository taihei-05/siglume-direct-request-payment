...
- Standard Payment の返金は merchant refund workflow API で扱います。この API は冪等性、返金可能額上限、監査、webhook、CSV、receipt 照合を扱いますが、返金送金そのものは merchant が自分の settlement wallet または適法な返金手段で実行します。Micro / Nano の調整は、アカウントで提供される Siglume support / platform process で扱います。
