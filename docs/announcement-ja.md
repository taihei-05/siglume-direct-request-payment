...
最短導入で案内できるのは、事前に merchant account、Hosted Checkout 有効化、billing mandate、公開 HTTPS webhook URL、buyer のテスト用 Siglume ウォレットがそろっている場合の **Standard Payment の初回テスト決済**です。本番運用、Micro / Nano 会計、subscription、scheduled autopay、返金対応は別途実装・確認が必要です。
