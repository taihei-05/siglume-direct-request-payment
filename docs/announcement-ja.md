...
- Hosted Checkout は beta 中のアカウント別有効化です。有効化依頼は [Hosted Checkout access request](https://github.com/taihei-05/siglume-direct-request-payment/issues/new?title=Hosted%20Checkout%20access%20request) から行ってください。
- 公開 GitHub issue は Hosted Checkout 有効化依頼、ドキュメント、SDK の一般的な不具合報告に使ってください。`request_id`、`trace_id`、`support_reference`、buyer ID、wallet address、token、取引固有情報は公開 issue に投稿しないでください。決済調査は private support / account contact で扱います。
- このSDK release には self-service refund API はありません。Standard Payment の返金や Micro / Nano の調整は、アカウントで提供される Siglume support / platform process で扱います。
...
Recurring support の現在の SDK 範囲は、challenge 生成、検証、webhook helper です。subscription / schedule の作成、更新、解約、失敗時再試行などの lifecycle 全体は高水準SDK methodではまだ完全には包んでいません。
